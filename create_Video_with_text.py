...
def add_audio_with_ffmpeg(video_no_audio: str, video_original: str, output_with_audio: str) -> bool:
    ...
    try:
        ...
        if not shutil.which("ffmpeg"):
            print("  ffmpeg no encontrado, intentando con moviepy...")
            return False
...
def add_audio_with_moviepy(video_no_audio: str, video_original: str, output_with_audio: str) -> bool:
    ...
    try:
        ...
        if audio is None:
            print("   El video original no tiene audio")
...
def create_video_with_text(
    ...
    if output_format == "mp4":
        ...
        if include_audio:
            ...
            if success:
                ...
                print(f" No se pudo agregar audio")
    ...
    if text_changes == 0:
        print(f"     ADVERTENCIA: No se detectaron cambios de texto")
    elif text_changes < len(audio_segments):
        print(f"    Se esperaban {len(audio_segments)} cambios pero solo se detectaron {text_changes}")
    else:
